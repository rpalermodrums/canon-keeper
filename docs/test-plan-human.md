...
- Bible entities/claims/evidence and confirm claim actions
...
- Dashboard shows worker status and zero-or-low queue depth
...
Expected:
- Last ingest stats populated
- Processing stages reach `ok`
- Scenes/Style/Issues/Bible show data
...
- Evidence drawer shows quote excerpt + chunk/document location
...
### 6.6 Bible + Confirmation Flow
1. Go to **Bible**.
2. Select an entity with inferred claims.
3. Open evidence for a claim.
4. Confirm claim via modal.

Expected:
- Confirm action succeeds
- Claim status reflects confirmation
...
Expected:
- Debounced re-ingest triggers automatically
- Queue depth spikes then returns
...
3. Visit Dashboard, Scenes, Style, Bible, Issues, Ask.
...
For 10 random claims/issues/scenes:
- Verify at least one quote span exists.
- Verify quote text is present in chunk excerpt.
- Verify location metadata (document path + chunk ordinal) is present.
